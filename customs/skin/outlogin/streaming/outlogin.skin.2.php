...
var win_coin = function(href) {
    var new_win = window.open(href, 'win_coin', 'left=100,top=100,width=600, height=600, scrollbars=1');
    new_win.focus();
}
$(function(){
    $(".win_coin").click(function() {
        win_coin(this.href);
        return false;
    });
});
        ...
        <li>
            <a href="<?php echo G5_SKIN_URL ?>/outlogin/streaming/coin.php" target="_blank" id="ol_after_coin" class="win_coin">
            엽전
            <strong>
            <?php
                $coin = get_coin_sum($member['mb_id']);
                if($coin == 0) echo 0;
                else echo $coin;
            ?>
            </strong>
